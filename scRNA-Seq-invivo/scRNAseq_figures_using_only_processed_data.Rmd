...
n_genes_differentially_expressed_per_celltype <- de_single_cell_dt[cell_type != "pseudo bulk"][padj<0.1][, .N , keyby = cell_type][is.na(N), N := 0]
n_genes_differentially_expressed_per_celltype[intersect(levels_of_cell_type, cell_type),]
de_single_cell_dt[cell_type != "pseudo bulk"] %>% ggplot(aes(x = cell_type, y = -log2FoldChange)) +
  ...
  scale_x_cell_type(labels = paste0(labels_of_cell_type, " [", n_genes_differentially_expressed_per_celltype[levels_of_cell_type, N], "]")) +
  scale_color_cell_type() +
  theme(axis.title.x = element_blank()) +
  NULL
```
...
r <- as(hex2RGB(maja_red),"polarLUV")@coords
b <- as(hex2RGB(maja_blue),"polarLUV")@coords
...
mhcII_features_dt[cell_type != "pseudo_bulk"] %>%
