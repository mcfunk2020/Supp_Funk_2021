...
```{r marker gene and score tSNE plots, fig.height=12, fig.width=7}
...
p_gene_s <- lapply(example_marker_genes, function(gene) {
  dt %>% ggplot(aes_string(x = "tSNE_1", y = "tSNE_2", color = gene)) + geom_cell() + scale_color_continuous(trans = "log1p", breaks = schelpr::log1p_breaks(TRUE, 10), name = paste0(gene, "\n[CPM]"))
})
p_score_s <- lapply(names(example_marker_genes), function(cell_type) {
  dt %>% ggplot(aes_string(x = "tSNE_1", y = "tSNE_2", color = paste0("`", cell_type, "`"))) + geom_cell() + scale_color_continuous(paste0(cell_type, "\nscore"))
})
...
```{r marker gene and score umap plots, fig.height=12, fig.width=7}
...
p_gene_s <- lapply(example_marker_genes, function(gene) {
  dt %>% ggplot(aes_string(x = "UMAP_1", y = "UMAP_2", color = gene)) + geom_cell() + scale_color_continuous(trans = "log1p", breaks = schelpr::log1p_breaks(TRUE, 10), name = paste0(gene, "\n[CPM]"))
})
p_score_s <- lapply(names(example_marker_genes), function(cell_type) {
  dt %>% ggplot(aes_string(x = "UMAP_1", y = "UMAP_2", color = paste0("`prediction.score.", cell_type, "`"))) + geom_cell() + scale_color_continuous(paste0(cell_type, "\nscore"))
})
