...
fgsea_hallmarktdy_exvivo <- read_delim("../../RNA-Seq-exvivo/Results/fgsea_hallmarktdy_exvivo.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
...
ggsave(cell_type_all_hallmarks, filename = "../Graphics/sc_cell_type_all_hallmarks.pdf", device = "pdf", bg = "white", width = 2600, height = 2600, units = "px")
...
ggsave(cell_type_selected_hallmarks, filename = "../Graphics/sc_cell_type_selected_hallmarks.pdf", device = "pdf", bg = "white", width = 2400, height = 2000, units = "px")
...
ggsave(dotplot_inverted, filename = "../Graphics/gsea_dotplot_inverted.pdf", device = "pdf", bg = "white", width = 2400, height = 2000, units = "px")
...
ggsave(correlation_plot, filename = "../Graphics/correlation_plot.pdf", device = "pdf", bg = "white")
```
...
shared_21_genes <- c("Cd74", "H2-Ab1", "1700011H14Rik", "H2-Eb1", "Muc2", "Irf1", "H2-Aa", "Ly6e", "Igtp", "Tifa", "H2-DMb1", "Psmb8", "Nrn1", "Prss32", "Ciita", "Sp140", "H2-DMa", "Oat", "H2-Q7", "Parp3", "Sepp1")

volcano_sc_pseudobulk <- de_single_cell_dt_raw %>%
  filter(contrast == "condition_young_vs_aged") %>% 
  filter(cell_type == "pseudo bulk") %>% 
  mutate(signif=if_else(p.adjusted<=0.1,if_else(-estimate>0,"up","down"),"none")) %>%
  tidyr::drop_na() %>% 
  ggplot(aes(x=-estimate,y=-log10(p.value),col=signif,name = gene)) +
    geom_point()+
    xlab("log2FoldChange") +
    ylab("-log10(pvalue)") +
    ggtitle("Aged_Over_Young_Top15_Pseudobulk") +
    scale_color_manual(values = c(maja_blue,b110_grey,maja_red)) +    
    geom_vline(xintercept = 0,lty=2) +
    geom_label_repel(data = de_single_cell_dt_raw %>% filter(contrast == "condition_young_vs_aged") %>% filter(cell_type == "pseudo bulk") %>% arrange(desc(abs(statistic))) %>% head(15), aes(label=gene), min.segment.length = 0, col="black") +
    theme_cowplot()

volcano_sc_pseudobulk

ggsave(volcano_sc_pseudobulk, filename = "../Graphics/volcano_sc_pseudobulk.pdf", device = "pdf", bg = "white")
...
volcano_sc_pseudobulk_21genes <- de_single_cell_dt_raw %>%
  ...
  ggplot(aes(x=-estimate,y=-log10(p.value),col=signif,name = gene)) +
    ...
    xlab("log2FoldChange") +
    ylab("-log10(pvalue)") +
    ggtitle("Aged_Over_Young_21_genes_in_common_Pseudobulk") +
    scale_color_manual(values = c(maja_blue,b110_grey,maja_red)) +
    geom_vline(xintercept = 0,lty=2) +
    geom_label_repel(data = de_single_cell_dt_raw %>% filter(gene %in% shared_21_genes) %>% filter(cell_type == "pseudo bulk"), aes(label=gene), min.segment.length = 0, col="black") +
    theme_cowplot()

volcano_sc_pseudobulk_21genes

ggsave(volcano_sc_pseudobulk_21genes, filename = "../Graphics/volcano_sc_pseudobulk_21genes.pdf", device = "pdf", bg = "white")
...
for (type in unique(de_single_cell_dt$cell_type)) {
  ...
  ggplot(aes(x=-log2FoldChange,y=-log10(pvalue),col=signif,name = gene)) +
    geom_point()+
    xlab("log2FoldChange") +
    ggtitle(paste0("Aged_Over_Young_Top20_", type)) +
    scale_color_manual(values = c(maja_blue,b110_grey,maja_red)) +
    geom_vline(xintercept = 0,lty=2) +
    geom_label_repel(data = de_single_cell_dt %>% filter(cell_type == type) %>% arrange(desc(abs(stat))) %>% head(20), aes(label=gene), min.segment.length = 0, col="black", max.overlaps = Inf) +
    theme_cowplot()
  print(volcano)
  
  ggsave(volcano, filename = paste0("../Graphics/volcano_sc_", type, ".pdf"), device = "pdf", bg = "white")
}
