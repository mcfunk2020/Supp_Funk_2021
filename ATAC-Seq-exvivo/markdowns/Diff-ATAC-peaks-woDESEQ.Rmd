...
```{r Wnt5a_example}
rm(A1,A1_ranges,A2,A2_ranges,A3,A3_ranges)
rm(Y1,Y1_ranges,Y2,Y2_ranges,Y3,Y3_ranges)
range_y <- c(0,1.5)

A_ranges <- import("raw_data/A_MEAN.mLb.clN.wig", as="GRanges") 

Y_ranges <- import("raw_data/Y_MEAN.mLb.clN.wig", as="GRanges")
...
pdf("graphics/Ciita_peak_large_zoom.pdf")
plotTracks(c(gtrack,ht),chromosome = chrom, 
           from = if(start_region<min(starts_highlight)){start_region}else{min(starts_highlight)-1000}, 
           to = if(end_region>max(end_highlight)){end_region}else{max(end_highlight)+1000} )
dev.off()
