...
Motif enrichment Analysis using Mona Lisa

The pipeline was adapted from:

Dania Machlab, Lukas Burger, Charlotte Soneson, Filippo M Rijli, Dirk Schübeler, Michael B Stadler, monaLisa: an R/Bioconductor package for identifying regulatory motifs, Bioinformatics, Volume 38, Issue 9, 1 May 2022, Pages 2624–2625, https://doi.org/10.1093/bioinformatics/btac102
...
hist(ATAC_exvivo_ranges$log2FoldChange, 100, col = "gray", main = "",
...
#set.seed(1)
lmrsel <- ATAC_exvivo_ranges#[ sample(x = length(ATAC_invivo_ranges), size = 10000, replace = FALSE) ]

#create bins and set a foldchange limit for unimportant regions
bins <- bin(x = ATAC_exvivo_ranges$log2FoldChange, binmode = "equalN", minAbsX = 0.2,nElements = 2000)
table(bins)
```
...
```{r}
se <- calcBinnedMotifEnrR(seqs = lmrseqs, bins = bins, pwmL = pwms, BPPARAM = BiocParallel::MulticoreParam(6))
